...
// /* ═══════════════════════════════════════════════════════════════════
//    FlyingRobot — pointer events handle both mouse and touch
// ═══════════════════════════════════════════════════════════════════ */
...
//   /* ── orbit centre — uses hero-right on desktop, hero-left on mobile ── */
//   const updateCentre = useCallback(() => {
//     const isMobile = window.innerWidth <= 1100;
//     // On mobile: float in bottom-right quadrant of viewport
//     if (isMobile) {
//       const size = robotRef.current?.offsetWidth ?? 120;
...
//           if (s.t >= Math.PI * 2) {
//             s.t = s.t % (Math.PI * 2);
//             s.paused = true; s.pauseLeft = PAUSE_MS;
//           }
...
//           const hp = orbitXY(s.homeT);
//           s.homeX = hp.px; s.homeY = hp.py;
//           const dx = s.homeX - s.px, dy = s.homeY - s.py;
...
//       setGlow(1);
//       // Pointer capture for mouse only (not supported on touch events)
...
//     /* Touch events — needed for mobile since pointer events can be unreliable */
...
//           {/* hero-right kept as orbit anchor on desktop; hidden on mobile */}
...
//         y:        Math.random() * canvas.height,
//         r:        Math.random() * 2 + 0.5,
//         // base drift direction — slow, cloud-like
//         baseVx:   Math.cos(angle) * speed,
//         baseVy:   Math.sin(angle) * speed,
//         // current velocity (perturbed by noise)
//         vx:       Math.cos(angle) * speed,
//         vy:       Math.sin(angle) * speed,
//         opacity:  Math.random() * 0.7 + 0.2,
//         color:    Math.random() > 0.5 ? "0,229,255" : "123,120,255",
//         pulse:    Math.random() * Math.PI * 2,
//         // noise phase offsets — each particle wanders independently
...
//       particles.forEach(p => {
//         // ── cloud-like wandering: perturb velocity with slow sine noise ──
...
//         // wrap around all four edges (not just top)
//         const margin = 20;
...
// /* ═══════════════════════════════════════════════════════════════════
//    FlyingRobot — with scroll-chase on mobile
// ═══════════════════════════════════════════════════════════════════ */
// function FlyingRobot({ imgSrc }) {
//   const robotRef = useRef(null);
...
//     const HOME_THRESH  = 6;
//     const MAX_THROW    = 1400;
//     const CHASE_SPRING = 5.0;
//     const CHASE_DAMP   = 0.75;
...
//       if (heroBottom < viewH * 0.2) {
//         s.scrollChasing  = true;
//         s.chaseTargetX   = window.innerWidth - size - 16;
//         s.chaseTargetY   = 16;
//         if (s.mode !== "scroll-chase") s.mode = "scroll-chase";
//       } else if (s.mode === "scroll-chase") {
//         s.scrollChasing = false;
//         s.homeT = nearestOrbitT();
//         const hp = orbitXY(s.homeT);
//         s.homeX = hp.px; s.homeY = hp.py;
//         s.mode = "homing";
...
//         case "scroll-chase": {
//           const dx = s.chaseTargetX - s.px;
//           const dy = s.chaseTargetY - s.py;
//           s.vx = (s.vx + dx * CHASE_SPRING * dt) * CHASE_DAMP;
//           s.vy = (s.vy + dy * CHASE_SPRING * dt) * CHASE_DAMP;
//           s.px += s.vx * dt;
//           s.py += s.vy * dt;
//           if (Math.abs(s.vx) > 8) s.faceDir = s.vx > 0 ? 1 : -1;
//           s.bobPhase += dt;
//           s.py += Math.sin(s.bobPhase * 1.2) * 0.4;
//           setGlow(0.2 + 0.1 * Math.sin(s.bobPhase * 2));
//           break;
//         }
...
//       const { clientX, clientY } = getClientXY(e);
//       s.dragging = true;
//       if (s.mode === "scroll-chase") s.scrollChasing = false;
//       s.vx = 0; s.vy = 0;
//       const size   = el.offsetWidth;
//       s.dragOffX   = clientX - (s.px + size / 2);
//       s.dragOffY   = clientY - (s.py + size / 2);
