...
// Bird pose sequence driven by image swaps + CSS class for position
const POSES = {
  SLIDE:   birdSlide,
...
export default function IntroAnimation({ onFinish }) {
  ...
  const [birdPhase, setBirdPhase] = useState("enter"); // CSS phase class

  useEffect(() => {
    const timers = [];
    const at = (fn, ms) => timers.push(setTimeout(fn, ms));

    // ── PHASE 1 : Bird slides in (0 → 600ms) ─────────────────
    ...
    // ── PHASE 2 : Logo + text appear (600ms → 1200ms) ─────────
    ...
    // Color swap to blue
    at(() => setIsBlue(true), 1350);

    // ── PHASE 4 : Celebrate — wing flap (1600ms) ──────────────
    at(() => {
    ...
    // ── PHASE 6 : Launch sequence (2400ms → 3000ms) ───────────
    at(() => {
    ...
    // ── PHASE 7 : Logo moves to navbar (2900ms) ───────────────
    at(() => {
